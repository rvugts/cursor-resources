...
- **Add more context** — user provides additional info (e.g. `@<path>` references to existing docs, code, or architecture diagrams)

Do NOT generate the spec until the user confirms. If Cursor is in **Plan Mode**, stay read-only until the user switches back to Agent Mode and confirms; only then write `docs/specs/spec.md`.
...
The spec MUST follow the template at `templates/spec-template.md` (attach with `@templates/spec-template.md` in Cursor so the model can align section-by-section). Read the full template before generating to ensure structural alignment.
...
This skill is the entry point for the SDD workflow shipped in this repo:
...
| Execute tasks | `/run-prompt` — runs prompts sequentially |

Notes for best results in Cursor:

- **Attach context with `@`.** Invite the user to attach existing architecture, ADRs, or code via `@docs/ARCHITECTURE.md`, `@src/…`, etc. during Phase 3 so questions are grounded in the real codebase.
- **Use Plan Mode for Phases 1–4.** It keeps the session read-only during intake, analysis, questioning, and the decision gate; switch to Agent Mode only for Phase 5 (generation and archival).
- **Template alignment.** The canonical template is `templates/spec-template.md`; the companion architecture template is `templates/ARCHITECTURE.md` — both live in this repo and can be attached with `@`.
- **Downstream chain.** After generation, recommend the user invoke `/create-tasks` next to produce `docs/specs/tasks.md`.
